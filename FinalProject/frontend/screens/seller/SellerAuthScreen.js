...
const signupSchema = Yup.object({
  storeName: Yup.string().min(2, 'Min 2 characters').required('Store name required'),
  email: Yup.string().email('Invalid email').required('Email required'),
  password: Yup.string().min(6, 'Min 6 characters').required('Password required'),
});

export default function SellerAuthScreen({ navigation }) {
  ...
  const handleLogin = async ({ email, password }) => {
    ...
    try {
      ...
      if (data.role !== 'seller') { setError('This account is not registered as a seller.'); return; }
      await loginUser({ id: cred.user.uid, email: cred.user.email, name: data.storeName || data.name || '', role: 'seller' });
    } catch { setError('Invalid credentials. Please try again.'); }
    finally { setLoading(false); }
  };

  const handleSignup = async ({ storeName, email, password }) => {
    setLoading(true); setError('');
    try {
      const cred = await createUserWithEmailAndPassword(auth, email, password);
      await setDoc(doc(db, 'users', cred.user.uid), { storeName, email, role: 'seller', createdAt: new Date().toISOString() });
      await loginUser({ id: cred.user.uid, email, name: storeName, role: 'seller' });
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
      <ScrollView style={{ backgroundColor: theme.bg }} contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
        <View style={[styles.header, { backgroundColor: '#4CAF50' }]}>
          ...
          <Text style={{ fontSize: 28, fontWeight: '800', color: '#fff', marginTop: 8 }}>Seller Portal</Text>
          <Text style={{ color: 'rgba(255,255,255,0.85)', marginTop: 4 }}>{isLogin ? 'Welcome back, Seller!' : 'Register your store'}</Text>
        </View>

        <View style={[styles.form, { backgroundColor: theme.card }]}>
          <Text style={[g.subtitle, { marginBottom: 18 }]}>{isLogin ? 'Login' : 'Register Store'}</Text>

          <Formik
            initialValues={isLogin ? { email: '', password: '' } : { storeName: '', email: '', password: '' }}
          ...
          >
            {({ handleChange, handleBlur, handleSubmit, values, errors, touched }) => (
              <View>
                {!isLogin && (
                  <>
                    <TextInput style={[g.input, { color: theme.text }]} placeholder="Store Name" placeholderTextColor={theme.subtext} value={values.storeName} onChangeText={handleChange('storeName')} onBlur={handleBlur('storeName')} />
                    {touched.storeName && errors.storeName && <Text style={g.errorText}>{errors.storeName}</Text>}
                  </>
                )}
                ...
                <TouchableOpacity style={[g.primaryButton, { backgroundColor: '#4CAF50', opacity: loading ? 0.7 : 1 }]} onPress={handleSubmit} disabled={loading}>
                  {loading ? <ActivityIndicator color="#fff" /> : <Text style={g.primaryButtonText}>{isLogin ? 'Login' : 'Register Store'}</Text>}
                </TouchableOpacity>
              </View>
            )}
          </Formik>
          ...
          <TouchableOpacity onPress={() => { setIsLogin(v => !v); setError(''); }} style={{ marginTop: 16, alignItems: 'center' }}>
            <Text style={{ color: '#4CAF50', fontWeight: '700' }}>
              {isLogin ? "Don't have a store account? Register" : 'Already have an account? Login'}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
